...
# Chave Vencedora (introduzida pelo utilizador)
print("Primeiro: Introduza a chave vencedora...")

winning_key = []
valid_num_key = False
valid_star_key = False

# Números da Chave
while valid_num_key != True:
    for item_number in range(5):
        key_item_number = int(input(f"Item {item_number+1} da chave vencedora (número de 1 a 50) -> "))
        if (1 <= key_item_number <= 50):
            ...
            key_item_number = int(input(f"Item {item_number+1} da chave vencedora (número de 1 a 50) -> "))
# Estrelas da Chave
while valid_star_key == False:
    for item_star in range(2):
        key_item_star = int(input(f"Estrela {item_star + 1} da chave vencedora (número entre 1 e 12) -> "))
        if (1 <= key_item_star <= 12):
            valid_star_key = True
...
# Descobrir se saiu vencedor e qual o prémio:
