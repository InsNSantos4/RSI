...
def check_ip(ip):
    ip = ip.strip()  # retira /n se existir
    ...
    for num in l_end: # percorre cada nummero
        if 0 > int(num) > 255:  # se num maior que 255 ou menor que 0 --> endereço inválido
            return False
    return True
...
for l in end_ok  :
    fout.write(l)    # escrita linha a linha da lista de endereços ok

fout.write('\n[Endereços inválidos:]\n')   # escrita do cabeçalho
for l in end_not_ok:
    fout.write(l)    # escrita linha a linha da lista de endereços not ok

fout.close()   # fecho do ficheiro para escrita

##################  Outro modo de escrita, utilizando o metodo writelines (imprime todos os elementos da lista)

fich_out = 'IP_out1.txt'
fout = open(fich_out,'w')

fout.write('[Endereços válidos:]\n')
fout.writelines(end_ok)

fout.write('\n[Endereços inválidos:]\n')
fout.writelines(end_not_ok)
